...
_REACT_PROMPT = (
    ...
    "    * 키워드 검색(예: '반도체', 'GPT')은 'naver' 우선, 결과 부족 시 'naver_crawl'.\n"
    "    * 'GeekNews' 명시 또는 IT 해커뉴스류 헤드라인은 'geeknews'.\n"
    ...
    "### Mode B — 평가\n"
    "먼저 한 문단(3~5문장) 자연어 요약, 빈 줄 한 칸 후 마크다운 목록.\n"
    "각 목록 줄은 다음 패턴을 그대로 따를 것 (단어 '제목'은 실제 기사 제목으로 치환):\n"
    "  `- [**Claude for Legal**](https://news.hada.io/topic?id=29557) · 중요도 4/5 — 법률 업무에 큰 변화 예상`\n"
    "즉 형식: `- [**<기사 제목>**](<URL>) · 중요도 N/5 — <한 줄 평가>`\n\n"
    "### Mode C — 종합 브리핑\n"
    "먼저 compose_brief 도구의 출력 텍스트를 한 단락으로 그대로 제시,\n"
    "빈 줄 한 칸 후 Mode B와 **동일한** 마크다운 목록 형식으로 각 기사 정리."
)
